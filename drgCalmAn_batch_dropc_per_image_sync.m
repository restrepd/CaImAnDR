%% drgCaImAn_batch_dropc_per_image_sync.m
%
% Used for the synchronization control and the auditory signal analysis
%
close all
clear all
...
for fileNo=handles_choice.first_file:handles_choice.no_files
     try
        ...
        %Read tiff file
        imageData=imread_big([handles_choice.PathNamecsv{fileNo} handles_choice.csvFileName{fileNo}]);
        num_imgs=size(imageData,3);
        traces=zeros(num_imgs,1);
        for imNo=1:num_imgs
            this_image=zeros(size(imageData,1),size(imageData,2));
            this_image(:,:)=imageData(:,:,imNo);
            traces(imNo,1)=mean(this_image(:));
        end
        traces=traces';
        ...
        set(hFig, 'units','normalized','position',[.05 .1 .85 .25])
 

        hold on

        % Determine the y spacing of the traces
        y_shift=1.2*prctile(traces(:),98);
        ...
        tic
        while at_end==0
            ii_next=find(image_ttl(ii:end)>1.5,1,'first');
            if ~isempty(ii_next)

                %found TTL
                ii_image=ii_image+1;
                ii=ii+ii_next-1;
                image_times(ii_image)=ii/acq_rate;
        ...
        while ii<length(image_times)
            jj=jj+1;
            image_times2(jj)=mean(image_times(ii:ii+no_scans_per_image-1));
        ...
        ylim([-y_shift*0.2 1.2*(prctile(traces(:),98)+y_shift)])
 

        xlabel('time (s)')
        ylabel('deltaF/F')
        title([fnameca(1:end-4) ' aligned to olfactometer'], 'Interpreter', 'none')
...
odor_on_times_rhd
        ...
        for trNo=1:no_traces
            % for trNo=1:20
            plot(image_times2,traces(trNo,1:length(image_times2))+y_shift*trNo,'-k','LineWidth',1)
        end
        ...
        all_dts=image_times2(2:end)-image_times2(1:end-1);
...
%         set(hFig, 'units','normalized','position',[.05 .05 .3 .85])
        ...
        %Extract S+ and S- using the rhd-synchronized data
        %For S+ and S- plot odor on and reinforcement
        trNo=0;
        ...
        for epoch=1:handles.dropcData.epochIndex
            ...
            if plot_epoch
                trNo=trNo+1;
                if (odor_on_times_rhd(trNo)-dt_before>image_times2(1))&(odor_on_times_rhd(trNo)+dt_after<image_times2(end))
                    snip_mask=(image_times2>=odor_on_times_rhd(trNo)-dt_before)...
                        &(image_times2<=odor_on_times_rhd(trNo)+dt_after);
                    no_points=sum(snip_mask)
                    if handles.dropcData.epochTypeOfOdor(epoch)==handles.dropcProg.splusOdor
                        spii=spii+1;
                        splus_traces_rhd(spii,1:no_points)=traces(1,snip_mask);
                    else
                        smii=smii+1;
                        sminus_traces_rhd(smii,1:no_points)=traces(1,snip_mask);
                    end
                end
            end
        end
        ...
        hold on
          ...
          %S+, S-, all snips
        CIsm = bootci(1000, @mean, sminus_traces_rhd);
        meansm=mean(sminus_traces_rhd,1);
        CIsm(1,:)=meansm-CIsm(1,:);
        CIsm(2,:)=CIsm(2,:)-meansm;

        CIsp = bootci(1000, @mean, splus_traces_rhd);
        meansp=mean(splus_traces_rhd,1);
        CIsp(1,:)=meansp-CIsp(1,:);
        CIsp(2,:)=CIsp(2,:)-meansp;

        szSp=size(splus_traces_rhd);
        szSm=size(sminus_traces_rhd);
       
        time_to_event=-dt_before+dt*[1:szSp(2)];
        pct1=prctile([mean(sminus_traces_rhd,1)'; mean(splus_traces_rhd(:,1:szSp(2)),1)'],1);
        pct99=prctile([mean(sminus_traces_rhd,1)'; mean(splus_traces_rhd(:,1:szSp(2)),1)'],99);

 

        [hlsm, hpsm] = boundedline(time_to_event',mean(sminus_traces_rhd,1)', CIsm', 'b');
        [hlsp, hpsp] = boundedline(time_to_event',mean(splus_traces_rhd,1)', CIsp', 'r');
        ...
        title("Ca changes aligned to odor onset, INTAN sycnhronized")
        ...
        title("Ca changes aligned to odor onset, olfactometer synchronized")
        legend([hlsp hlsm odorhl reinfhl],'S+','S-','Odor','Reinforcement')
        xlabel('Time (sec)')
        ylabel('dF/F')
        ylim([pct1-0.2*(pct99-pct1) pct99+0.2*(pct99-pct1)])
