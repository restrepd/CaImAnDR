...
clus_per_peak_ii=[1 2]; %Note I am using only one peak per cluster
...
for fNo=1:length(handles_out.file)
    if  isfield(handles_out.file(fNo).output_data_odor,'total_trials')
...
files_included_glm=zeros(1,length(handles_out.file));
for fNo=1:length(handles_out.file)
    if isfield(handles_out.file(fNo).output_data_odor_Sp,'total_trials')
...
%             title(['Histogram for divergence time for ' per_names{grNo+1}])
    ...
    try
        ...
        input_data(2).data=divergence_times_per_group.group(3).div_t(divergence_times_per_group.group(3).div_t<=7)';
        ...
        fprintf(1,['Number of learning divergence times ' num2str(length(input_data(1).data)), ' \n'])
        fprintf(1,['Number of proficient divergence times ' num2str(length(input_data(2).data)), ' \n'])
    catch
    end
