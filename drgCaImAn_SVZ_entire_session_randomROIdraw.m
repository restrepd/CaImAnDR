function handles_out2=drgCaImAn_SVZ_entire_session_randomROIdraw(handles_choices)
...
for iiROI=1:no_ROI_draws+1
    if iiROI==1
        %Get an ROI limited to p<=p_thrshold
        p_value_mask=logical(p_values<=p_threshold);
        p_value_masks(1,:)=p_value_mask;
        no_ROIs=sum(p_value_mask);
    else
        %Get a random set of the same number of ROIs as in iiROI==1 
        found_it=0;
        while found_it==0
