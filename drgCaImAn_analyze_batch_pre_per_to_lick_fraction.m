%drgCaImAn_analyze_batch_pre_per_to_lick_fraction
close all
clear all
...
fileID = fopen([choiceBatchPathName 'drgCaImAn_analyze_batch_pre_per_to_lick_fraction.txt'],'w');
...
time_periods_lf_label{1}='Pre';
time_periods_lf_label{2}='Odor';
time_periods_lf_label{3}='Reinf';
...
%                     handles_out2.group_no(grNo).KLdivergence(ii_euclid,1:length(handles_out.ii_out(1).handles_out.KLdivergence))=handles_out.ii_out(1).handles_out.KLdivergence;
                ...
                for ii_out=1:length(handles_out.ii_out)
                    ...
                    %Now do S+ licks
                    this_mean_per_trial_sp_lick_timecourse=zeros(length(these_sp_hits),length(handles_out.ii_out(1).handles_out.time_span));
                    if  sum(these_sp_hits)>0
                        this_mean_per_trial_sp_lick_timecourse(1:sum(these_sp_hits),:)=this_mean_per_trial_hit_lick_timecourse;
                    end
                    if  sum(these_sp_miss)>0
                        this_mean_per_trial_sp_lick_timecourse(sum(these_sp_hits)+1:end,:)=this_mean_per_trial_miss_lick_timecourse;
                    end
                    if (sum(these_sp_hits)>0)||(sum(these_sp_miss)>0)
                        this_mean_mean_per_trial_sp_lick_timecourse=mean(this_mean_per_trial_sp_lick_timecourse);
                        handles_out2.group_no(grNo).ii_thr(ii_out).MLalgo(iiMLalgo).mean_per_trial_sp_lick_timecourse(ii,1:length(this_mean_mean_per_trial_sp_lick_timecourse))=this_mean_mean_per_trial_sp_lick_timecourse;
                    else
                        handles_out2.group_no(grNo).ii_thr(ii_out).MLalgo(iiMLalgo).sp_included(ii)=0;
                    end
                    ...
                    %Now do S- licks
                    this_mean_per_trial_sm_lick_timecourse=zeros(length(these_sm_crs),length(handles_out.ii_out(1).handles_out.time_span));
                    if  sum(these_sm_crs)>0
                        this_mean_per_trial_sm_lick_timecourse(1:sum(these_sm_crs),:)=this_mean_per_trial_cr_lick_timecourse;
                    end
                    if  sum(these_sm_fas)>0
                        this_mean_per_trial_sm_lick_timecourse(sum(these_sm_crs)+1:end,:)=this_mean_per_trial_fa_lick_timecourse;
                    end

                    if (sum(these_sm_crs)>0)||(sum(these_sm_fas)>0)
                        this_mean_mean_per_trial_sm_lick_timecourse=mean(this_mean_per_trial_sm_lick_timecourse);
                        handles_out2.group_no(grNo).ii_thr(ii_out).MLalgo(iiMLalgo).mean_per_trial_sm_lick_timecourse(ii,1:length(this_mean_mean_per_trial_sm_lick_timecourse))=this_mean_mean_per_trial_sm_lick_timecourse;
                    else
                        handles_out2.group_no(grNo).ii_thr(ii_out).MLalgo(iiMLalgo).sm_included(ii)=0;
                    end
        ...
        if ~isempty(handles_out2.group_no(grNo).ii_time_span)
            ...
            for ii_f=1:size(handles_out2.group_no(grNo).ii_time_span,1)
                if these_mice(ii_f)==mouseNo
                    ...
                    %Now do the lick calculation for sm
                    ii_sm_included=ii_sm_included+1;
                    for ii_tsp=1:ii_tspan

                        if time_span(ii_tsp)>this_time_span(end)
                            these_licks_mean_sm(ii_sm_included,ii_tsp)=handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sm_lick_timecourse(ii_f,end);
                        else
                            ii_0=find(this_time_span<=time_span(ii_tsp),1,'last');
                            ii_1=find(this_time_span>time_span(ii_tsp),1,'first');
                            these_licks_mean_sm(ii_sm_included,ii_tsp)=handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sm_lick_timecourse(ii_f,ii_0)+...
                                (handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sm_lick_timecourse(ii_f,ii_1)...
                                -handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sm_lick_timecourse(ii_f,ii_0))...
                                *(time_span(ii_tsp)-this_time_span(ii_0))/(this_time_span(ii_1)-this_time_span(ii_0));
                        end
                    end
            ...
            per_mouse_pred.group(grNo).mouse(mouseNo).licks_sm=these_licks_mean_sm;
            ...
            for ii_f=1:size(handles_out2.group_no(grNo).ii_time_span,1)
                if these_mice(ii_f)==mouseNo
                    ...
                    %Now do the lick calculation for sp
                    ii_sp_included=ii_sp_included+1;
                    for ii_tsp=1:ii_tspan

                        if time_span(ii_tsp)>this_time_span(end)
                            these_licks_mean_sp(ii_sp_included,ii_tsp)=handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sp_lick_timecourse(ii_f,end);
                        else
                            ii_0=find(this_time_span<=time_span(ii_tsp),1,'last');
                            ii_1=find(this_time_span>time_span(ii_tsp),1,'first');
                            these_licks_mean_sp(ii_sm_included,ii_tsp)=handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sp_lick_timecourse(ii_f,ii_0)+...
                                (handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sp_lick_timecourse(ii_f,ii_1)...
                                -handles_out2.group_no(grNo).ii_thr(ii_thr).MLalgo(iiMLalgo).mean_per_trial_sp_lick_timecourse(ii_f,ii_0))...
                                *(time_span(ii_tsp)-this_time_span(ii_0))/(this_time_span(ii_1)-this_time_span(ii_0));
                        end
                    end
                    pffft=1;
            ...
            per_mouse_pred.group(grNo).mouse(mouseNo).licks_sp=these_licks_mean_sp;
...
%         end
        ...
        if include_mouse==1
            ii_mouse_inc_hit=ii_mouse_inc_hit+1;
            per_mouse_licks_sp_hit(ii_mouse_inc_hit,:)=mean(this_mouse_licks_sp_hit,1);
            mouse_no_hit(ii_mouse_inc_hit)=mouseNo;
        end
        ...
        if include_mouse==1
            ii_mouse_inc_miss=ii_mouse_inc_miss+1;
            mouse_no_miss(ii_mouse_inc_miss)=mouseNo;
        ...
        if include_mouse==1
            ii_mouse_inc_cr=ii_mouse_inc_cr+1;
            mouse_no_cr(ii_mouse_inc_cr)=mouseNo;
        ...
        if include_mouse==1
            ii_mouse_inc_fa=ii_mouse_inc_fa+1;
            mouse_no_fa(ii_mouse_inc_fa)=mouseNo;
            per_mouse_licks_sm_fa(ii_mouse_inc_fa,:)=mean(this_mouse_licks_sm_fa,1);
        end

        %Extract sp licks per mouse
        ii_sessions_inc=0;
        this_mouse_licks_sp=[];
        include_mouse=0;

        for grNo=these_gr_out
            if ~isempty(per_mouse_pred.group(grNo).mouse(mouseNo).licks_sp)
                for ii_sessions=1:size(per_mouse_pred.group(grNo).mouse(mouseNo).licks_sp,1)
                    ii_sessions_inc=ii_sessions_inc+1;
                    this_mouse_licks_sp(ii_sessions_inc,:)=per_mouse_pred.group(grNo).mouse(mouseNo).licks_sp(ii_sessions,:);
                end
                include_mouse=1;
            end
        end
        if include_mouse==1
            ii_mouse_inc_sp=ii_mouse_inc_sp+1;
            per_mouse_licks_sp(ii_mouse_inc_sp,:)=mean(this_mouse_licks_sp,1);
        end


        %Extract sm licks per mouse
        ii_sessions_inc=0;
        this_mouse_licks_sm=[];
        include_mouse=0;

        for grNo=these_gr_out
            if ~isempty(per_mouse_pred.group(grNo).mouse(mouseNo).licks_sm)
                for ii_sessions=1:size(per_mouse_pred.group(grNo).mouse(mouseNo).licks_sm,1)
                    ii_sessions_inc=ii_sessions_inc+1;
                    this_mouse_licks_sm(ii_sessions_inc,:)=per_mouse_pred.group(grNo).mouse(mouseNo).licks_sm(ii_sessions,:);
                end
                include_mouse=1;
            end
        end
        if include_mouse==1
            ii_mouse_inc_sm=ii_mouse_inc_sm+1;
            per_mouse_licks_sm(ii_mouse_inc_sm,:)=mean(this_mouse_licks_sm,1);
        end

    end

    %Do summary accounting for the bar graph
    for time_epoch=1:size(time_periods_lf,1)
        this_lf=mean(per_mouse_licks_sp_hit(:,(time_span>=time_periods_lf(time_epoch,1))&(time_span<=time_periods_lf(time_epoch,2)))');
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_hit=this_lf;
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_hit=mouse_no_hit;

        this_lf=mean(per_mouse_licks_sp_miss(:,(time_span>=time_periods_lf(time_epoch,1))&(time_span<=time_periods_lf(time_epoch,2)))');
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_miss=this_lf;
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_miss=mouse_no_miss;

        this_lf=mean(per_mouse_licks_sm_cr(:,(time_span>=time_periods_lf(time_epoch,1))&(time_span<=time_periods_lf(time_epoch,2)))');
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_cr=this_lf;
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_cr=mouse_no_cr;

        this_lf=mean(per_mouse_licks_sm_fa(:,(time_span>=time_periods_lf(time_epoch,1))&(time_span<=time_periods_lf(time_epoch,2)))');
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_fa=this_lf;
        lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_fa=mouse_no_fa;
    end

    %Plot time course for hit, miss, cr and fa
    figureNo = figureNo + 1;
    try
        close(figureNo)
    catch
    ...
    %FV
    plot([-1.5 0],[this_ylim(1)+0.05*(this_ylim(2)-this_ylim(1)) this_ylim(1)+0.05*(this_ylim(2)-this_ylim(1))],'LineWidth',5, Color=[0.9 0.9 0.9])
    plot([-1.5 -1.5],[this_ylim],'-','Color',[0.5 0.5 0.5])

    %Odor on markers
    plot([0 0],this_ylim,'-k')
    odorhl=plot([0 delta_odor],[this_ylim(1)+0.05*(this_ylim(2)-this_ylim(1)) this_ylim(1)+0.05*(this_ylim(2)-this_ylim(1))],'-k','LineWidth',5);
    plot([delta_odor delta_odor],this_ylim,'-k')

    %Reinforcement markers
    plot([delta_odor_on_reinf_on delta_odor_on_reinf_on],this_ylim,'-r')
    reinfhl=plot([delta_odor_on_reinf_on delta_odor_on_reinf_on+delta_reinf],[this_ylim(1)+0.05*(this_ylim(2)-this_ylim(1)) this_ylim(1)+0.05*(this_ylim(2)-this_ylim(1))],'-r','LineWidth',5);
    plot([delta_odor_on_reinf_on+delta_reinf delta_odor_on_reinf_on+delta_reinf],this_ylim,'-r')


    title(['Lick fraction for ' per_corr_set_label{pCorr_no} ', hit=red, miss=cyan, fa=magenta, cr=blue'])
    xlabel('Time(sec)')
    ylabel('Lick fraction')
% 
%     %Plot time course for sp and sm
%     figureNo = figureNo + 1;
%     try
%         close(figureNo)
%     catch
...
% 
%       %Plot sm
%     if size(per_mouse_licks_sm,1)>=3
%         CIpv = bootci(1000, @mean, per_mouse_licks_sm);
%         meanpv=mean(per_mouse_licks_sm,1);
%         CIpv(1,:)=meanpv-CIpv(1,:);
%         CIpv(2,:)=CIpv(2,:)-meanpv;
% 
%         [hlpvl, hppvl] = boundedline(time_span',mean(per_mouse_licks_sm,1)', CIpv', 'cmap',[158/255 31/255 99/255]);
%     else
%         plot(time_span',mean(per_mouse_licks_sm,1)', '-','Color',[158/255 31/255 99/255]);
%     end
% 
%     %Plot sp
%     if size(per_mouse_licks_sp,1)>=3
%         CIpv = bootci(1000, @mean, per_mouse_licks_sp);
%         meanpv=mean(per_mouse_licks_sp,1);
%         CIpv(1,:)=meanpv-CIpv(1,:);
%         CIpv(2,:)=CIpv(2,:)-meanpv;
% 
%         [hlpvl, hppvl] = boundedline(time_span',mean(per_mouse_licks_sp,1)', CIpv','cmap',[0 114/255 178/255]);
%     else
%         plot(time_span',mean(per_mouse_licks_sp,1)', '-r');
%     end
% 
% 
%     plot(time_span',mean(per_mouse_licks_sm,1)', '-','Color',[158/255 31/255 99/255]);
%     plot(time_span',mean(per_mouse_licks_sp,1)', '-','Color',[0 114/255 178/255]);
% 
% 
%     ylim([-0.1 1.1])
%     this_ylim=ylim;
...
% 
% 
%     title(['Lick fraction for proficient'])
%     xlabel('Time(sec)')
%     ylabel('Lick fraction')
...
for time_epoch=1:size(time_periods_lf,1)
    ...
    for pCorr_no=these_pcorr
        ...
        %Do fa
        these_licks=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_fa;
        glm_lick.data(glm_lick_ii+1:glm_lick_ii+length(these_licks))=these_licks;
        glm_lick.pcorr(glm_lick_ii+1:glm_lick_ii+length(these_licks))=pCorr_no*ones(1,length(these_licks));
        glm_lick.trial_type(glm_lick_ii+1:glm_lick_ii+length(these_licks))=0*ones(1,length(these_licks));
        glm_lick.mouse_nos(glm_lick_ii+1:glm_lick_ii+length(these_licks))=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_hit;
        glm_lick_ii=glm_lick_ii+length(these_licks);

        id_lick_ii=id_lick_ii+1;
        input_lick_data(id_lick_ii).data=these_licks;
        input_lick_data(id_lick_ii).description=['FA ' per_corr_set_label{pCorr_no}];


        %FA
        bar(bar_offset,mean(these_licks),'LineWidth', 3,'EdgeColor','none','FaceColor','m')

        %Violin plot
        if ~isempty(these_licks)
            [mean_out, CIout]=drgViolinPoint(these_licks...
                ,edges,bar_offset,rand_offset,'k','k',2);
        end
        ...
        %Do cr
        these_licks=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_cr;
        glm_lick.data(glm_lick_ii+1:glm_lick_ii+length(these_licks))=these_licks;
        glm_lick.pcorr(glm_lick_ii+1:glm_lick_ii+length(these_licks))=pCorr_no*ones(1,length(these_licks));
        glm_lick.trial_type(glm_lick_ii+1:glm_lick_ii+length(these_licks))=1*ones(1,length(these_licks));
        glm_lick.mouse_nos(glm_lick_ii+1:glm_lick_ii+length(these_licks))=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_cr;
        glm_lick_ii=glm_lick_ii+length(these_licks);

        id_lick_ii=id_lick_ii+1;
        input_lick_data(id_lick_ii).data=these_licks;
        input_lick_data(id_lick_ii).description=['CR ' per_corr_set_label{pCorr_no}];


        %CR
        bar(bar_offset,mean(these_licks),'LineWidth', 3,'EdgeColor','none','FaceColor','b')
        ...
        %Do miss
        these_licks=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_miss;
        glm_lick.data(glm_lick_ii+1:glm_lick_ii+length(these_licks))=these_licks;
        glm_lick.pcorr(glm_lick_ii+1:glm_lick_ii+length(these_licks))=pCorr_no*ones(1,length(these_licks));
        glm_lick.trial_type(glm_lick_ii+1:glm_lick_ii+length(these_licks))=2*ones(1,length(these_licks));
        glm_lick.mouse_nos(glm_lick_ii+1:glm_lick_ii+length(these_licks))=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_miss;
        glm_lick_ii=glm_lick_ii+length(these_licks);

        id_lick_ii=id_lick_ii+1;
        input_lick_data(id_lick_ii).data=these_licks;
        input_lick_data(id_lick_ii).description=['Miss ' per_corr_set_label{pCorr_no}];


        %CR
        bar(bar_offset,mean(these_licks),'LineWidth', 3,'EdgeColor','none','FaceColor','c')
        ...
        %Do hit
        these_licks=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).lick_f_hit;
        glm_lick.data(glm_lick_ii+1:glm_lick_ii+length(these_licks))=these_licks;
        glm_lick.pcorr(glm_lick_ii+1:glm_lick_ii+length(these_licks))=pCorr_no*ones(1,length(these_licks));
        glm_lick.trial_type(glm_lick_ii+1:glm_lick_ii+length(these_licks))=3*ones(1,length(these_licks));
        glm_lick.mouse_nos(glm_lick_ii+1:glm_lick_ii+length(these_licks))=lick_f_summary.pcorr(pCorr_no).epoch(time_epoch).mouse_no_fa;
        glm_lick_ii=glm_lick_ii+length(these_licks);

        id_lick_ii=id_lick_ii+1;
        input_lick_data(id_lick_ii).data=these_licks;
        input_lick_data(id_lick_ii).description=['Hit ' per_corr_set_label{pCorr_no}];


        %CR
        bar(bar_offset,mean(these_licks),'LineWidth', 3,'EdgeColor','none','FaceColor','r')
    ...
    xticks([1.5 6.5])
    xticklabels({'Learning','Proficient'})


    title(['Lick fraction,' time_periods_lf_label{time_epoch} ' hit=red, miss=cyan, fa=magenta, cr=blue'])
    ylabel('Lick fraction')
    ylim([0 1.1])
    xlim([-1 9])
    ...
    %Perform the glm for licks
    fprintf(1, ['\nglm for decoding lick fraction ' time_periods_lf_label{time_epoch} '\n'])
    fprintf(fileID, ['\nglm for decoding lick fraction ' time_periods_lf_label{time_epoch} '\n']);

    % fprintf(1, ['\nwindow 0: Base, 1:PreFV, 2:PreOdor, 3:Odor\n'])
    % fprintf(fileID, ['\nwindow 0: Base, 1:PreFV, 2:PreOdor, 3:Odor\n']);

    tbl = table(glm_lick.data',glm_lick.pcorr',glm_lick.trial_type',glm_lick.mouse_nos',...
        'VariableNames',{'lick_f','pcorr','trial_type','mice'});
    mdl = fitglm(tbl,'lick_f~pcorr+trial_type+mice+trial_type*pcorr'...
        ,'CategoricalVars',[2,3,4])
    ...
    %Do the ranksum/t-test
    fprintf(1, ['\n\nRanksum or t-test p values for decoding lick fraction ' time_periods_lf_label{time_epoch} '\n'])
    fprintf(fileID, ['\n\nRanksum or t-test p values for decoding lick fraction ' time_periods_lf_label{time_epoch} '\n']);
