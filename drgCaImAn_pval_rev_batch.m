function handles_out=drgCaImAn_pval_rev_batch(handles_choices)
...
use_pFDR=3; %0=use t-test or ranksum with p<p_value_sig, 1=use pFDR, 2=use bootstrapping, 3= glm
p_value_sig=0.001;
...
%     end
    ...
    if forward_proficient_divergent.alROIii_inc>5
        %Calculate the crosscorrelations
        %     croscorr_traces=abs(corrcoef(handles_out.all_div_dFFspm)); %please note that I am using the absolute value
        ...
        xlim([1 forward_proficient_divergent.alROIii_inc])
        ylim([1 forward_proficient_divergent.alROIii_inc])
        ...
        caxis([prctile(handles_out.all_div_fp_dFFOdor12_forward(:),1) prctile(handles_out.all_div_fp_dFFOdor12_forward(:),99)])

        for ii_te=1:length(timeEvents)
            plot([timeEvents(ii_te) timeEvents(ii_te)],[0 forward_proficient_divergent.alROIii_inc],'-r')
        end
