function drgCaImAn_batch_analysis_pre_per_to_inspect_fsdzv2
...
if all_files_present==1
    ...
    for ii_pc_group=[1 3]
        ...
        per_session_delta_zdist_euclid=zeros(200,length(trimmed_time_span));
        per_session_delta_zdist_euclid_within=zeros(200,length(trimmed_time_span));
        ii_included=0;
        for ii_mouse=1:length(these_mice)
            ...
            if ii_movies>0
                ...
                per_session_delta_zdist_euclid(ii_included+1:ii_included+ii_movies,:)=these_delta_zdist_euclid';
                per_session_delta_zdist_euclid_within(ii_included+1:ii_included+ii_movies,:)=these_delta_zdist_euclid_within';
                ii_included=ii_included+ii_movies;
            end

        end
        
        per_session_delta_zdist_euclid(ii_included+1:end,:)=[];
        per_session_delta_zdist_euclid_within(ii_included+1:end,:)=[];
        ...
        CIeu = bootci(1000, @mean, per_session_delta_zdist_euclid_within);
        meaneu=mean(per_session_delta_zdist_euclid_within,1);
        ...
        CIeu = bootci(1000, @mean, per_session_delta_zdist_euclid);
        meaneu=mean(per_session_delta_zdist_euclid,1);
        CIeu(1,:)=meaneu-CIeu(1,:);
        CIeu(2,:)=CIeu(2,:)-meaneu;
...
%     title('Euclidean distance between-within')
    ...
    mdl = fitglm(tbl,'euclidean_distance~p_correct+time_window+mice+between_vs_within+p_correct*time_window*between_vs_within'...
    ...
    for ii_pc_group=[1 3]
        ...
        per_session_meandFF_sp=zeros(200,length(trimmed_time_span));
        per_session_mean_dFF_sm=zeros(200,length(trimmed_time_span));
        ii_included=0;
        ...
        for ii_mouse=1:length(these_mice)
            ...
            if ii_movies>0
                ...
                per_session_meandFF_sp(ii_included+1:ii_included+ii_movies,:)=these_meandFF_sp';
                per_session_meandFF_sm(ii_included+1:ii_included+ii_movies,:)=these_meandFF_sm';
                ii_included=ii_included+ii_movies;
            end

        end

        per_session_meandFF_sp(ii_included+1:end,:)=[];
        per_session_meandFF_sm(ii_included+1:end,:)=[];
        ...
        %S+ trials
        try
            CIsp = bootci(1000, @mean, per_session_meandFF_sp);
            meansp=mean(per_session_meandFF_sp,1);
        ...
        %S-
        try
            CIsm = bootci(1000, @mean, per_session_meandFF_sm);
            meansm=mean(per_session_meandFF_sm,1);
...
%         ,'CategoricalVars',[2,3,4,5])
    ...
    tbl = table(glm_dFF.data',glm_dFF.pcorr',glm_dFF.window',glm_dFF.spm',glm_dFF.mice',...
        'VariableNames',{'mean_dFF','p_correct','time_window','sp_vs_sm','mice'});
    mdl = fitglm(tbl,'mean_dFF~p_correct+mice+time_window+sp_vs_sm+p_correct*time_window*sp_vs_sm'...
        ,'CategoricalVars',[2,3,4,5])
...
%     
    ...
    for ii_pc_group=[1 3]
        ...
        p_val_per_mouse=[];
        p_val_per_session=zeros(200,sum((time_p_lick>=-7)&(time_p_lick<=15)));
        sp_lick_freq_per_session=zeros(200,sum((time_p_lick>=-7)&(time_p_lick<=15)));
        sm_lick_freq_per_session=zeros(200,sum((time_p_lick>=-7)&(time_p_lick<=15)));
        ii_included=0;
        min_trials=16;
        ...
        for ii_mouse=1:length(these_mice)
            ...
            if ii_movies>0
                ...
                for ii_m=1:ii_movies
                    these_lick_sp=all_data.mouse(ii_mouse).p_corr(ii_pc_group).movie(ii_movies).lick_sp(:,(time_p_lick>=-7)&(time_p_lick<=15));
                    these_lick_sm=all_data.mouse(ii_mouse).p_corr(ii_pc_group).movie(ii_movies).lick_sm(:,(time_p_lick>=-7)&(time_p_lick<=15));
                    
                    if size(these_lick_sp,1)+size(these_lick_sm,1)>=min_trials
                        ii_included=ii_included+1;
                        for no_pv=1:length(trimmed_time_p_lick)
                            this_Sm=zeros(1,size(these_lick_sm,1));
                            this_Sm(1,:)=these_lick_sm(:,no_pv);
                            this_Sp=zeros(1,size(these_lick_sp,1));
                            this_Sp(1,:)=these_lick_sp(:,no_pv);
                            if (~isempty(this_Sm))&(~isempty(this_Sp))
                                p_val_per_session(ii_included,no_pv)=log10(ranksum(this_Sm,this_Sp));
                            else
                                p_val_per_session(ii_included,no_pv)=log10(1);
                            end

                            %ranksum gives NaN if the values are all the same
                            if isnan(p_val_per_session(ii_included,no_pv))
                                p_val_per_session(ii_included,no_pv)=log10(1);
                            end

                            %Calculate lick fraction
                            sp_lick_freq_per_session(ii_included,no_pv)=sum(this_Sp)/(length(this_Sp));
                            sm_lick_freq_per_session(ii_included,no_pv)=sum(this_Sm)/(length(this_Sm));
                ...
                for no_pv=1:length(trimmed_time_p_lick)
                    ...
                    %Calculate lick fraction
                    sp_lick_freq_per_mouse(no_pv,ii_mouse_included)=sum(this_Sp)/(length(this_Sp));
                    sm_lick_freq_per_mouse(no_pv,ii_mouse_included)=sum(this_Sm)/(length(this_Sm));
        ...
        sp_lick_freq_per_session(ii_included:end,:)=[];
        sm_lick_freq_per_session(ii_included:end,:)=[];
        p_val_per_session(ii_included:end,:)=[];
        ...
        %S+ trials
        try
            CIsp = bootci(1000, @mean, sp_lick_freq_per_session);
            meansp=mean(sp_lick_freq_per_session,1);
            CIsp(1,:)=meansp-CIsp(1,:);
            CIsp(2,:)=CIsp(2,:)-meansp;
        ...
        %S-
        try
            CIsm = bootci(1000, @mean, sm_lick_freq_per_session);
            meansm=mean(sm_lick_freq_per_session,1);
            CIsm(1,:)=meansm-CIsm(1,:);
            CIsm(2,:)=CIsm(2,:)-meansm;
        ...
        xlim([-7 15])
        ax.LineWidth=3;
        title(['Lick fraction ' prof_labels{ii_pc_group}])
        xlabel('Time(sec)')
        ...
        try
            CIeu = bootci(1000, @mean, p_val_per_session);
            meaneu=mean(p_val_per_session,1);
            CIeu(1,:)=meaneu-CIeu(1,:);
            CIeu(2,:)=CIeu(2,:)-meaneu;
...
%      Perform glm and generate bar graphs for lick fraction
        glm_frac=[];
        glm_ii=0;
        ...
        for ii_pc_group=[1 3]
            ...
            for ii_t_period=1:4
                ...
                for ii_mouse=1:length(these_mice)
                    ii_movies=all_data.mouse(ii_mouse).p_corr(ii_pc_group).ii_movies;
                    if ii_movies>0
                        these_frac_sp=zeros(1,ii_movies);
                        these_frac_sm=zeros(1,ii_movies);
                        for ii_m=1:ii_movies
                            this_lick_f_sp=mean(all_data.mouse(ii_mouse).p_corr(ii_pc_group).movie(ii_m).lick_sp,1);
                            these_frac_sp(1,ii_m)=mean(this_lick_f_sp...
                                ((time_p_lick>=time_periods_eu(ii_t_period,1))&(time_p_lick<=time_periods_eu(ii_t_period,2))));
                             this_lick_f_sm=mean(all_data.mouse(ii_mouse).p_corr(ii_pc_group).movie(ii_m).lick_sm,1);
                            these_frac_sm(1,ii_m)=mean(this_lick_f_sm...
                                ((time_p_lick>=time_periods_eu(ii_t_period,1))&(time_p_lick<=time_periods_eu(ii_t_period,2))));
                        end

                        per_mouse_frac_sp(:,ii_mouse)=mean(these_frac_sp,2);
                        per_mouse_frac_sm(:,ii_mouse)=mean(these_frac_sm,2);
                        all_frac_sp=[all_frac_sp these_frac_sp];
                        all_frac_sm=[all_frac_sm these_frac_sm];
                    end


                    glm_frac.data(glm_ii+1:glm_ii+length(these_frac_sp))=these_frac_sp;
                    glm_frac.pcorr(glm_ii+1:glm_ii+length(these_frac_sp))=ii_pc_group*ones(1,length(these_frac_sp));
                    glm_frac.window(glm_ii+1:glm_ii+length(these_frac_sp))=ii_t_period*ones(1,length(these_frac_sp));
                    glm_frac.spm(glm_ii+1:glm_ii+length(these_frac_sp))=1*ones(1,length(these_frac_sp));
                    glm_frac.mice(glm_ii+1:glm_ii+length(these_frac_sp))=ii_mouse*ones(1,length(these_frac_sp));
                    glm_ii=glm_ii+length(these_frac_sp);
    
                    glm_frac.data(glm_ii+1:glm_ii+length(these_frac_sm))=these_frac_sm;
                    glm_frac.pcorr(glm_ii+1:glm_ii+length(these_frac_sm))=ii_pc_group*ones(1,length(these_frac_sm));
                    glm_frac.window(glm_ii+1:glm_ii+length(these_frac_sm))=ii_t_period*ones(1,length(these_frac_sm));
                    glm_frac.spm(glm_ii+1:glm_ii+length(these_delta_zdist_euclid))=0*ones(1,length(these_delta_zdist_euclid));
                    glm_frac.mice(glm_ii+1:glm_ii+length(these_frac_sm))=ii_mouse*ones(1,length(these_frac_sm));
                    glm_ii=glm_ii+length(these_frac_sm);
                end
    
                id_ii=id_ii+1;
                input_data(id_ii).data=all_frac_sp;
                input_data(id_ii).description=[period_labels{ii_t_period} ' S+ ' prof_labels{ii_pc_group}];
    
                id_ii=id_ii+1;
                input_data(id_ii).data=all_frac_sm;
                input_data(id_ii).description=[period_labels{ii_t_period} ' S- ' prof_labels{ii_pc_group}];
    

                %Basline bar S-
                bar_offset=bar_offset+1;
                bar(bar_offset,mean(all_frac_sm),'LineWidth', 3,'EdgeColor','none','FaceColor',[238/255 111/255 179/255])
    
                if length(all_frac_sm)>2
                    %Violin plot
                    [mean_out, CIout]=drgViolinPoint(all_frac_sm...
                        ,edges,bar_offset,rand_offset,'k','k',1);
                end
                ...
                %Baseline bar S+
                bar_offset=bar_offset+1;
                bar(bar_offset,mean(all_frac_sp),'LineWidth', 3,'EdgeColor','none','FaceColor',[80/255 194/255 255/255])
    
                if length(all_frac_sp)>2
                    %Violin plot
                    [mean_out, CIout]=drgViolinPoint(all_frac_sp...
                        ,edges,bar_offset,rand_offset,'k','k',1);
                end
    
                for ii_mouse=1:length(per_mouse_frac_sp)
                    plot([bar_offset-1 bar_offset],[per_mouse_frac_sm(ii_mouse) per_mouse_frac_sp(ii_mouse)],'-','Color',[150/255 150/255 150/255],'LineWidth',2 )
                end
            end
            bar_offset=bar_offset+1;
        end
        ...
        fprintf(1, ['\nglm for lick fraction\n'])
        fprintf(fileID, ['\nglm for lick fraction\n']);
    
        tbl = table(glm_frac.data',glm_frac.pcorr',glm_frac.window',glm_frac.spm',glm_frac.mice',...
            'VariableNames',{'mean_frac','p_correct','time_window','sp_vs_sm','mice'});
        mdl = fitglm(tbl,'mean_frac~p_correct+time_window+sp_vs_sm+mice+p_correct*time_window*sp_vs_sm'...
            ,'CategoricalVars',[2,3,4,5])
        ...
        fprintf(fileID, ['\n\nRanksum or t-test p values for mean lick fraction\n']);
    
    
        [output_data] = drgMutiRanksumorTtest(input_data, fileID,0);

%     %Do glm for per mouse lick fraction
%     glm_frac=[];
%     glm_ii=0;
% 
%     id_ii=0;
...
%                     this_frac_sp=[];
%                     this_frac_sm=[];
%                     this_frac_sp=mean(per_mouseLR.pcorr(ii_pc_group).mouse(ii_mouse).sp_lick_freq_per_mouse...
%                         ((trimmed_time_p_lick>=time_periods_eu(ii_t_period,1))&(trimmed_time_p_lick<=time_periods_eu(ii_t_period,2))));
%                     this_frac_sm=mean(per_mouseLR.pcorr(ii_pc_group).mouse(ii_mouse).sm_lick_freq_per_mouse...
%                         ((trimmed_time_p_lick>=time_periods_eu(ii_t_period,1))&(trimmed_time_p_lick<=time_periods_eu(ii_t_period,2))));
% 
% 
%                     glm_frac.data(glm_ii+1)=this_frac_sp;
%                     glm_frac.pcorr(glm_ii+1)=ii_pc_group;
%                     glm_frac.window(glm_ii+1)=ii_t_period;
%                     glm_frac.spm(glm_ii+1)=1;
%                     glm_frac.mice(glm_ii+1)=ii_mouse;
%                     glm_ii=glm_ii+1;
% 
%                     glm_frac.data(glm_ii+1)=this_frac_sm;
%                     glm_frac.pcorr(glm_ii+1)=ii_pc_group;
%                     glm_frac.window(glm_ii+1)=ii_t_period;
%                     glm_frac.spm(glm_ii+1)=0;
%                     glm_frac.mice(glm_ii+1)=ii_mouse;
%                     glm_ii=glm_ii+1;
% 
%                     ii_mice_included=ii_mice_included+1;
%                     all_frac_sp(ii_mice_included)=this_frac_sp;
%                     all_frac_sm(ii_mice_included)=this_frac_sm;
%                 end
%             end
% 
%             id_ii=id_ii+1;
...
%             end
% 
%             for ii_mouse=1:length(all_frac_sp)
%                 plot([bar_offset-1 bar_offset],[all_frac_sm(ii_mouse) all_frac_sp(ii_mouse)],'-','Color',[150/255 150/255 150/255],'LineWidth',2 )
%             end
%         end
%         bar_offset=bar_offset+1;
%     end
...
% 
%     title('')
%     ylabel('lick fraction (Hz)')
% 
% 
% 
%     %Perform the glm
%     fprintf(1, ['\nglm for mean lick fraction per mouse\n'])
%     fprintf(fileID, ['\nglm for mean lick fraction per mouse\n']);
% 
%     tbl = table(glm_frac.data',glm_frac.pcorr',glm_frac.window',glm_frac.spm',...
%         'VariableNames',{'mean_LF','p_correct','time_window','sp_vs_sm'});
%     mdl = fitglm(tbl,'mean_LF~p_correct+time_window+sp_vs_sm+p_correct*time_window*sp_vs_sm'...
%         ,'CategoricalVars',[2,3,4])
% 
% 
%     txt = evalc('mdl');
...
% 
%     %Do the ranksum/t-test
%     fprintf(1, ['\n\nRanksum or t-test p values for mean lcik fraction\n'])
%     fprintf(fileID, ['\n\nRanksum or t-test p values for mean lick fraction\n']);
% 
% 
%     [output_data] = drgMutiRanksumorTtest(input_data, fileID,0);
