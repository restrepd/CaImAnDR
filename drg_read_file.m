function imData=drg_read_file(path_to_file,sframe,num2read,options,im_info)
...
elseif strcmpi(ext,'.nd2')
    meta=drg_imreadBFmeta(path_to_file);
    reader = bfGetReader(path_to_file);
    imData=zeros(meta.width,meta.height,meta.nframes);
    if isinf(num2read)
        no_frames=meta.nframes;
    else
        no_frames=num2read;
    end
    for ii_frame=1:no_frames
        imData(:,:,ii_frame)=bfGetPlane(reader, ii_frame);
        ii_frame
    end
else
