function handles_out=drgCaImAn_SVZ_entire_session_errors(handles_choices)
...
drgCaImAn_add_trimmed_licks_to_pre_per(pre_perFileName,pre_perPathName);
                ...
                if ((ii+next_ii-ii_span)>0)&((ii+next_ii+ii_span<length(label_traces)))
                    if this_sp_sm==1
                        ...
                        if epochs(ii+next_ii)==6
                            ...
                            per_trial_hit_licks(hit_ii,:)=trimmed_licks(ii+next_ii-ii_span:ii+next_ii+ii_span);
                            these_hits(tr_ii)=1;
                            these_miss(tr_ii)=0;
                        ...
                        if epochs(ii+next_ii)==7
                            ...
                            per_trial_miss_licks(miss_ii,:)=trimmed_licks(ii+next_ii-ii_span:ii+next_ii+ii_span);
                            these_hits(tr_ii)=0;
                            these_miss(tr_ii)=1;
                        ...
                        if epochs(ii+next_ii)==9
                            ...
                            per_trial_cr_licks(cr_ii,:)=trimmed_licks(ii+next_ii-ii_span:ii+next_ii+ii_span);
                            these_crs(tr_ii)=1;
                            these_fas(tr_ii)=0;
                        ...
                        if epochs(ii+next_ii)==8
                            ...
                            per_trial_fa_licks(fa_ii,:)=trimmed_licks(ii+next_ii-ii_span:ii+next_ii+ii_span);
                            these_crs(tr_ii)=0;
                            these_fas(tr_ii)=1;
...
%         end
        ...
        handles_out.MLalgo(MLalgo).per_trial_hit_licks=per_trial_hit_licks;
        handles_out.MLalgo(MLalgo).per_trial_miss_licks=per_trial_miss_licks;
        handles_out.MLalgo(MLalgo).per_trial_cr_licks=per_trial_cr_licks;
        handles_out.MLalgo(MLalgo).per_trial_fa_licks=per_trial_fa_licks;

        moving_mean_per_trial_hit_licks = movmean(per_trial_hit_licks',moving_mean_n)';
        moving_mean_per_trial_miss_licks = movmean(per_trial_miss_licks',moving_mean_n)';
        moving_mean_per_trial_cr_licks = movmean(per_trial_cr_licks',moving_mean_n)';
        moving_mean_per_trial_fa_licks = movmean(per_trial_fa_licks',moving_mean_n)';
...
%             subplot(2,1,1)
        ...
        %Plot Hits
            ...
            meansm=mean(moving_mean_per_trial_sm_licks,1);
            CIsm(1,:)=meansm-CIsm(1,:);
            CIsm(2,:)=CIsm(2,:)-meansm;

            [hlsm, hpsm] = boundedline(time_span',mean(moving_mean_per_trial_sm_licks,1)', CIsm', 'cmap',[158/255 31/255 99/255]);

            CIsp = bootci(1000, @mean, moving_mean_per_trial_sp_licks);
            meansp=mean(moving_mean_per_trial_sp_licks,1);
            CIsp(1,:)=meansp-CIsp(1,:);
            CIsp(2,:)=CIsp(2,:)-meansp;


            [hlsp, hpsp] = boundedline(time_span',mean(moving_mean_per_trial_sp_licks,1)', CIsp', 'cmap',[0 114/255 178/255]);

            plot(time_span',mean(moving_mean_per_trial_sm_licks,1)','-','Color',[158/255 31/255 99/255],'DisplayName','S-')
            plot(time_span',mean(moving_mean_per_trial_sp_licks,1)', '-','Color',[0 114/255 178/255],'DisplayName','S+');
            ...
            %Plot the lick timecourse for hit, miss, cr, fa
            figNo=figNo+1;
            try
                close(figNo)
            catch
            ...
            %Plot hits
            if size(moving_mean_per_trial_hit_licks,1)>=3
                CIpv = bootci(1000, @mean, moving_mean_per_trial_hit_licks);
                meanpv=mean(moving_mean_per_trial_hit_licks,1);
                CIpv(1,:)=meanpv-CIpv(1,:);
                CIpv(2,:)=CIpv(2,:)-meanpv;

                [hlpvl, hppvl] = boundedline(time_span',mean(moving_mean_per_trial_hit_licks,1)', CIpv', '-r');
            else
                plot(time_span',mean(moving_mean_per_trial_hit_licks,1)', '-r');
            end

            %Plot miss
            if size(moving_mean_per_trial_miss_licks,1)>=3
                CIpv = bootci(1000, @mean, moving_mean_per_trial_miss_licks);
                meanpv=mean(moving_mean_per_trial_miss_licks,1);
                CIpv(1,:)=meanpv-CIpv(1,:);
                CIpv(2,:)=CIpv(2,:)-meanpv;

                [hlpvl, hppvl] = boundedline(time_span',mean(moving_mean_per_trial_miss_licks,1)', CIpv', '-c');
            else
                plot(time_span',mean(moving_mean_per_trial_miss_licks,1)', '-c');
            end

            %Plot cr
            if size(moving_mean_per_trial_cr_licks,1)>=3
                CIpv = bootci(1000, @mean, moving_mean_per_trial_cr_licks);
                meanpv=mean(moving_mean_per_trial_cr_licks,1);
                CIpv(1,:)=meanpv-CIpv(1,:);
                CIpv(2,:)=CIpv(2,:)-meanpv;

                [hlpvl, hppvl] = boundedline(time_span',mean(moving_mean_per_trial_cr_licks,1)', CIpv', '-b');
            else
                plot(time_span',mean(moving_mean_per_trial_cr_licks,1)', '-b');
            end

            %Plot fa
            if size(moving_mean_per_trial_fa_licks,1)>=3
                CIpv = bootci(1000, @mean, moving_mean_per_trial_fa_licks);
                meanpv=mean(moving_mean_per_trial_fa_licks,1);
                CIpv(1,:)=meanpv-CIpv(1,:);
                CIpv(2,:)=CIpv(2,:)-meanpv;

                [hlpvl, hppvl] = boundedline(time_span',mean(moving_mean_per_trial_fa_licks,1)', CIpv', '-m');
            else
                plot(time_span',mean(moving_mean_per_trial_fa_licks,1)', '-m');
            end
