function handles_out=drgCaImAn_dPCA_entire_sessionv2(handles_choices)
...
dPCA_input=1; %if this is 1 it will precess data from a pre_per file
                %if it is 0 it will process simulated data
...
fprintf(1, ['\nDirectory ' strrep(pre_perPathName, '\', '\\') '\n' ]);
fprintf(1, ['\ndrgCaImAn_dPCA_entire_sessionv2 run for ' pre_perFileName '\n\n']);
...
if dPCA_input==1
    ...
    %     for s=1:S
    %         if s==0
    %             trialNum(n,s)=sum(sp_or_sm==0);
    %         else
    %             trialNum(n,s)=sum(sp_or_sm==1);
    %         end
    %     end
    % end
    ...
    for trNum=1:length(sp_or_sm)
        s=sp_or_sm(trNum)+1;
        for n=1:N
            trialNum(n,s)=trialNum(n,s)+1;
            firingRates(n,s,:,trialNum(n,s))=measurements_per_trial(:,n,trNum);
        end
    end

else
    %This is a simulation with a stimulus dependent change from 10 to 20
    %and a stimulus independent change from 15 to 25
    N = 100;%100;   % number of neurons
    T = 30;%20;     % number of time points
    S = 2;%7;       % number of stimuli
    E = 25;%20;     % maximal number of trial repetitions
    ...
    for n=1:N
        for s=1:S
            for trNum=1:E
                firingRates(n,s,:,trNum)=SD_odor*randn(1,T);
            end
        end
    end

    %Half trials are odor 1

    %A tenth of the cells respond to odor 1 or odor 2 or the memory of odor 1
    %or 2
    for n=1:ceil(N/3)
        %Odor dependent
        s=1;
        for trNum=1:E
            firingRates(n,s,odor_dt,trNum)=firingRates(n,s,odor_dt,trNum)+odor_response;
        end

        %odor independent
        
        for trNum=1:E
            firingRates(n+ceil(N/3),1,odor_dt+5,trNum)=firingRates(n,s,odor_dt,trNum)+odor_response;
            firingRates(n+ceil(N/3),2,odor_dt+5,trNum)=firingRates(n,s,odor_dt,trNum)+odor_response;
        end
