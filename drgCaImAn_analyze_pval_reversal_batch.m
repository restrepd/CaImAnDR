...
file_grNo=zeros(1,length(handles_out.file));
for fileNo=1:length(handles_out.file)
    file_grNo(fileNo)=handles_out.file(fileNo).grNo;
end
    ...
    for mouseNo=unique(handles_out.mouseNo)
        ...
        switch handles.group_algo
            case 1
                ...
                switch grNo
                    case 1
                        files_included=(handles_out.perCorr>=80)&(handles_out.mouseNo==mouseNo)&files_included_glm&(file_grNo==1);
                    case 3
                        files_included=(handles_out.perCorr>=80)&(handles_out.mouseNo==mouseNo)&files_included_glm&(file_grNo==2);
                end
            case 2
                %Fabio
...
%     end
    ...
    for grNo=[1 3]
        ...
        switch grNo
            case 1
                ROIs_included=(handles_out.all_div_dFFspm_pcorr>=80)&(handles_out.all_div_dFFspm_group==1);
            case 3
                ROIs_included=(handles_out.all_div_dFFspm_pcorr>=80)&(handles_out.all_div_dFFspm_group==2);
        end
...
%             croscorr_traces=corrcoef(these_all_div_dFFspm);
            ...
            for clus=1:max(clusters)
                ...
                if grNo==1
                    title(['dFF divergent forward, cluster no ' num2str(clus)])
                else
                    title(['dFF divergent reversed, cluster no ' num2str(clus)])
                end
...
%     clus_ii_legends{4}='cluster 3';
    ...
    for clus_ii=1:3
        ...
        title(['zdFF for cluster no ' num2str(clus_ii)])



        %Perform the glm for this cluster
        fprintf(1, ['\nglm for zdFF for cluster no '  num2str(clus_ii) '\n'])
        fprintf(fileID, ['\nglm for zdFF for cluster no '  num2str(clus_ii) '\n']);

        tbl = table(glm_clus.data',glm_clus.odor',glm_clus.fwdrev',...
            'VariableNames',{'zdFF','odor','froward_reversed'});
        mdl = fitglm(tbl,'zdFF~odor+froward_reversed+odor*froward_reversed'...
            ,'CategoricalVars',[2,3])
        ...
        %Do the ranksum/t-test
        fprintf(1, ['\n\nRanksum or t-test p values for zdFF for cluster no ' num2str(clus_ii) '\n'])
        fprintf(fileID, ['\n\nRanksum or t-test p values for zdFF for cluster no ' num2str(clus_ii) '\n']);
    ...
    %Percent per cluster forward and reversed 
    id_cluschi_ii=0;
    input_cluschi_data=[];
    figureNo=figureNo+1;
    try
    ...
    for clus_no=1:3
        for grNo=[1 3]
            ...
            %Do S+
            no_ROIs_in_this_cluster=size(dFF_timecourse_per_clus.group(grNo).cluster(clus_no).splus_timecourses,1);
            this_percent=100*no_ROIs_in_this_cluster/total_ROIs_this_cluster;

            id_cluschi_ii=id_cluschi_ii+1;
            input_cluschi_data(id_cluschi_ii).data=[no_ROIs_in_this_cluster total_ROIs_this_cluster];
            input_cluschi_data(id_cluschi_ii).description=[naive_pro{grNo} ' cluster no ' num2str(clus_no)];

            bar_offset=bar_offset+1;
            bar(bar_offset,this_percent,'LineWidth', 3,'EdgeColor','none','FaceColor',[150/255 150/255 150/255])
        end
        bar_offset=bar_offset+1;
    end
%  
