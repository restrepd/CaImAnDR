function handles_out=drgCaImAn_dPCA_stim_within_group_batchv2(handles_choices)
...
%This will  process data for one group at a time where
%grNo=1 is 45-65
%grNo=3 is >=80
...
if all_files_present==1
    ...
    for fileNo=first_file:length(handles.FileName_pre_per)
        ...
        fprintf(1, ['Loading data from file number %d\n'],fileNo);
        ...
        all_handles(fileNo).handles_out=drgCaImAn_get_dFF_per_trialv3(handles_choices);
...
for grNo=[1 3]
    ...
    ii_mouse_included=0;
    for mouseNo=1:ii_mouse
        if ~isempty(handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out)
            ii_mouse_included=ii_mouse_included+1;
            per_mouse_meandPCAcomp_sp(ii_mouse_included,:)=handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out.sum_stimulus_comp(2,:)/mean_odor_sp_component(mouseNo);
        end
    end
    ...
    for mouseNo=1:ii_mouse
        if ~isempty(handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out)
            ii_mouse_included=ii_mouse_included+1;
            per_mouse_meandPCAcomp_sm(ii_mouse_included,:)=handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out.sum_stimulus_comp(1,:)/mean_odor_sp_component(mouseNo);
        end
    end
    ...
    plot(time',meansm','-','Color',[238/255 111/255 179/255])
    plot(time',meansp','-','Color',[80/255 194/255 255/255])
    ...
    for mouseNo=1:ii_mouse
        if ~isempty(handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out)
            ii_mouse_included=ii_mouse_included+1;
            per_mouse_meandPCAindcomp_sp(ii_mouse_included,:)=handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out.sum_independent_comp(2,:)/mean_odor_sp_component(mouseNo);
        end
    end
    ...
    for mouseNo=1:ii_mouse
        if ~isempty(handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out)
            ii_mouse_included=ii_mouse_included+1;
            per_mouse_meandPCAindcomp_sm(ii_mouse_included,:)=handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out.sum_independent_comp(1,:)/mean_odor_sp_component(mouseNo);
        end
    end
    ...
    for ii_t_period=1:size(time_periods_eu,1)
        ...
        for mouseNo=1:ii_mouse
            if ~isempty(handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out)
                ...
                per_mouse_meanstimdPCA_sp(ii_mouse_included)=mean(this_sp_component((time>=time_periods_eu(ii_t_period,1))&(time<=time_periods_eu(ii_t_period,2))))/mean_odor_sp_component(mouseNo);

                this_sm_component=[];
                this_sm_component=handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out.sum_stimulus_comp(1,:);
                per_mouse_meanstimdPCA_sm(ii_mouse_included)=mean(this_sm_component((time>=time_periods_eu(ii_t_period,1))&(time<=time_periods_eu(ii_t_period,2))))/mean_odor_sp_component(mouseNo);

                this_sp_component=[];
                this_sp_component=handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out.sum_independent_comp(2,:);
                per_mouse_meaninddPCA_sp(ii_mouse_included)=mean(this_sp_component((time>=time_periods_eu(ii_t_period,1))&(time<=time_periods_eu(ii_t_period,2))))/mean_odor_sp_component(mouseNo);

                this_sm_component=[];
                this_sm_component=handles_dpca.perpCorr.mouse(mouseNo).group(grNo).handles_out.sum_independent_comp(1,:);
                per_mouse_meaninddPCA_sm(ii_mouse_included)=mean(this_sm_component((time>=time_periods_eu(ii_t_period,1))&(time<=time_periods_eu(ii_t_period,2))))/mean_odor_sp_component(mouseNo);
            end
        end
...
%Perform glm
fileID = fopen([choiceBatchPathName 'drgCaImAn_dPCA_stim_within_group_batch.txt'],'w');

%Perform the glm for stimulus
fprintf(1, ['\nglm for dPCA stimulus component\n'])
fprintf(fileID, ['\nglm dPCA stimulus component\n']);
...
fprintf(fileID, ['\nglm dPCA stimulus-independent component\n']);
...
save([handles.choiceBatchPathName handles.choiceFileName(1:end-4) suffix_out],'handles_dpca','handles','-v7.3')
pffft=1;